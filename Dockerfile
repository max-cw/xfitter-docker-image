...
RUN yum -y install gcc-c++ gcc-gfortran bzip2 libpng libjpeg \
    python-devel boost-devel libSM libX11 libXext libXpm libXft gsl-devel python-pip make git which wget redhat-lsb-core \
    && yum -y clean all
...
#Install xfitter
ADD https://gitlab.cern.ch/maxcw/xfitter/blob/master/tools/install-xfitter /var/tmp/install-xfitter.sh
